...
def soa(freq_dict,cut_off = 5):
	...
	for collocate in freq_dict["combined_freq"]:
		...
		if freq_dict["combined_freq"][collocate] >= cut_off: #check to make sure that the collocate occurs frequently enough to surpass the threshold
			...
			faith_coll_cue[collocate] = (a/(a+b)) - (c/(c+d)) #P(target | collocate) - P(target | -collocate)
...
```
The results here indicate that when "bureau" occurs in the corpus, a form of "investigat" will occur 18.59% of the time.
```
bureau  0.18592512318092905
original        0.04841933281595099
federal 0.0405309679246142
report  0.04010749388937588
committee       0.03559090825130142
city    0.012598254202815625
number  0.010468818529035447
used    0.009696535833430726
other   0.006932895337473269
been    0.006358725928974626
```
